...
# 编辑报告信息页面
@manage_report_bp.route('/edit_report/<int:InstitutionID>', methods=['GET', 'POST'])
@login_required
def edit_report(InstitutionID):
    ...
    if request.method == 'GET':
        return render_template('edit_report.html', report=institution, institution_type=institution_type,
                               institution_name=institution_name)

    # 如果是 POST 请求，更新举报信息
    new_report_info = request.form['report_info']
    institution.ReportInfo = new_report_info  # 更新 ReportInfo 字段
    db.session.commit()

    return redirect(url_for('manage_report.manage_report'))

# 删除报告信息（仅清空 ReportInfo 字段）
@manage_report_bp.route('/delete_report/<int:InstitutionID>', methods=['GET'])
@login_required
def delete_report(InstitutionID):
    institution_type, institution = get_institution_type_by_id(InstitutionID)
